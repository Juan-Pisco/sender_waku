...
export class ENRTree {
    ...
    static parseAndVerifyRoot(root, publicKey) {
        assert(root.startsWith(this.ROOT_PREFIX), `ENRTree root entry must start with '${this.ROOT_PREFIX}'`);
        ...
        const isVerified = ecdsaVerify(signatureBuffer, keccak256Buf(signedComponentBuffer), keyBuffer);
    ...
    static parseTree(tree) {
        assert(tree.startsWith(this.TREE_PREFIX), `ENRTree tree entry must start with '${this.TREE_PREFIX}'`);
    ...
    static parseBranch(branch) {
        assert(branch.startsWith(this.BRANCH_PREFIX), `ENRTree branch entry must start with '${this.BRANCH_PREFIX}'`);
